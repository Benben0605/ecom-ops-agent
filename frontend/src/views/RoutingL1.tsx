...
const percentOptional = (value: number | null | undefined) => value == null ? "—" : percent(value);
...
const runPassed = (item: RoutingCase) => `${count((item.pass_rate ?? 0) * (item.n ?? 1))} / ${item.n ?? 1}`;
...
function L1Drawer({ item, onClose }: { item: RoutingCase | null; onClose: () => void }) {
  ...
  return <CaseDrawer open={Boolean(item)} title={item ? `${item.case_id} · ${item.bucket}` : ""} subtitle={item && <div className="badge-row">{item.route_error && <VerdictBadge verdict="route_error" />}{item.is_misfire && <VerdictBadge verdict="misfire" />}{item.not_run && <VerdictBadge verdict="not_run" />}</div>} onClose={onClose}>
    {item && <>
      <section className="drawer-question"><span>评估问题</span><p>{item.question}</p><div className="badge-row">{isExperiment && <span className="annotation-pill">pass {percentOptional(item.pass_rate)} · {runPassed(item)}</span>}{item.missing_tools.map((tool) => <span className="tool-badge missing" key={`missing-${tool}`}>missing · {tool}</span>)}{item.unexpected_tools.map((tool) => <span className="tool-badge unexpected" key={`unexpected-${tool}`}>unexpected · {tool}</span>)}</div></section>
      ...
      {isExperiment && <details className="drawer-section" open><summary><span>N</span>Run 稳定性 <small>{item.n} runs</small></summary><div className="details-content">
        <div className="run-grid">
          {item.experiment_runs?.map((run) => <div className={`run-card ${run.passed ? "ok" : "bad"}`} key={run.run_index}>
            <div className="run-card-head"><b>run_{run.run_index}</b><span>{run.passed ? "pass" : "fail"}</span></div>
            <div className="definition-grid compact"><span>called</span><p>{tools(run.called_tools)}</p><span>hit</span><b>{String(run.is_hit)}</b><span>misfire</span><b>{String(run.is_misfire)}</b></div>
            <div className="badge-row">{run.missing_tools.map((tool) => <span className="tool-badge missing" key={`run-${run.run_index}-missing-${tool}`}>missing · {tool}</span>)}{run.unexpected_tools.map((tool) => <span className="tool-badge unexpected" key={`run-${run.run_index}-unexpected-${tool}`}>unexpected · {tool}</span>)}</div>
          </div>)}
        </div>
      </div></details>}
...
export default function RoutingL1({ query }: RoutingProps) {
  ...
  return <div className="page-stack">
    ...
    <section className="compact-metrics"><MetricCard compact label={isExperiment ? "实验样本数" : "评估覆盖率"} value={isExperiment ? String(data.metrics.case_count) : percent(data.metrics.coverage_rate)} note={`${data.metrics.evaluated_case_count} / ${data.metrics.case_count}`} /><MetricCard compact label="路由准确率" value={percent(data.metrics.routing_accuracy)} note={`${count(data.metrics.route_hit_count)} / ${data.metrics.positive_case_count}`} /><MetricCard compact label="误触发率" value={percent(data.metrics.misfire_rate)} note={`${count(data.metrics.misfire_count)} / ${data.metrics.evaluated_case_count}`} /><MetricCard compact label="失败 case" value={String(data.metrics.failure_case_count)} note={isExperiment ? "任一 run 失败即计入" : "route error 或 misfire"} /></section>
    ...
    <section className="panel"><div className="panel-heading case-heading"><div><span className="section-kicker">Trace</span><h2>Case 追溯</h2></div><div className="segmented"><button type="button" className={!showAll ? "active" : ""} onClick={() => setShowAll(false)}>仅失败 <b>{failures.length}</b></button><button type="button" className={showAll ? "active" : ""} onClick={() => setShowAll(true)}>全部 <b>{data.cases.length}</b></button></div></div>
      <div className="table-scroll case-table"><table><thead><tr><th>case</th><th>bucket</th><th className="question-column">问题</th><th>{isExperiment ? "稳定性" : "裁决"}</th><th>期望 → 实际</th><th /></tr></thead><tbody>{list.map((item) => <tr className="clickable-row" key={item.case_id} onClick={() => setSelected(item)}><td><b className="case-id">{item.case_id}</b></td><td><span className="bucket-name">{item.bucket}</span></td><td className="truncate-cell" title={item.question}>{item.question}</td><td><div className="badge-row">{isExperiment && <span className={item.pass_rate === 1 ? "rate-pill good" : "rate-pill bad"}>pass {percentOptional(item.pass_rate)}</span>}{item.route_error && <VerdictBadge verdict="route_error" />}{item.is_misfire && <VerdictBadge verdict="misfire" />}{item.not_run && <VerdictBadge verdict="not_run" />}{item.is_hit && !item.is_misfire && <VerdictBadge verdict="hit" />}</div></td><td className="tool-route"><span>{tools(item.expected_tools)}</span><em>→</em><span>{tools(item.called_tools)}</span></td><td className="row-arrow">›</td></tr>)}</tbody></table>{!list.length && <div className="empty-state"><span>✓</span><b>当前筛选下没有失败 case</b><p>切换“全部”可浏览完整评估集。</p></div>}</div>
    </section>
