...
_system_prompt = "你是私域电商运营客服助手，负责订单查询、售后/政策咨询、商品推荐和运营数据分析，善于利用工具解决问题。必填参数齐全时直接调用工具，不要因为选填参数缺失而反问；但若必填参数缺失且无法从对话中合理推断，应先向用户询问澄清，不要猜测或随意填充必填参数。如果是电商业务相关但无工具，调用 escalate_to_human 转人工，不要硬调最接近的工具兜底。"
...
class SupervisorAgent:
    ...
    ┌─────────────────────────────────────────────────────────────────────┐
    │ 你要实现的契约（三条硬要求，缺一条 eval 就对不上 / 审计就乱）：        │
    │                                                                       │
    │ 1. supervisor 自己的 tool-loop 只认两个 agent-as-tool                  │
    │    （ASK_CUSTOMER_AGENT_SCHEMA / ASK_MERCHANT_AGENT_SCHEMA）。          │
    │    复用 ChatSession 即可：tool_schemas=[两个schema]、                  │
    │    tool_impls={"ask_customer_agent": <dispatch>, ...}。                │
    │                                                                       │
    │ 2. 【只记叶子】supervisor 这层的 dispatch 调用【不进审计流】           │
    │    → 给 supervisor 的 ChatSession 注入 NoOpRecorder()。                │
    │    真正的业务工具(query_order...)由专家记，用【真 recorder】。         │
    │                                                                       │
    │ 3. 【审计穿线】专家 Agent 必须用 supervisor 的 session_id 记审计，     │
    │    否则 eval 按 run_map(case_id→supervisor.id) JOIN 不到叶子。         │
    │    → dispatch 里 new ChatSession(..., session_id=self.id,              │
    │      audit_recorder=<真 recorder>, tool_schemas=专家子集,             │
    │      tool_impls=专家子集)，调 .chat(query) 返回文本当 tool 结果。      │
    └─────────────────────────────────────────────────────────────────────┘
    """
    ...
    def __init__(self, audit_recorder=None, message_recorder=None, session_id: str | None = None):
        ...
        # TODO(you): 构造两个 dispatch 闭包 ask_customer_agent(query)/ask_merchant_agent(query)
        #   —— 每个闭包内：用专家子集 + self._leaf_recorder + session_id=self.id 建 ChatSession，
        #      跑 .chat(query)，return 专家的最终文本。
        # TODO(you): self._session = ChatSession(SUPERVISOR_PROMPT,
    ...
    @property
    def messages(self):
        # TODO(you): 返回 supervisor 那条会话的 messages（self._session.messages）
