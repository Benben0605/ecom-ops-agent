...
from src.agent import ChatSession, SupervisorAgent
...
def eval_answer_run(
    make_agent=_default_make_agent,
    ...
    for case in eval_cases:
        session = make_agent(
            AuditRecorder(recorder_path=run / "audit.jsonl"), _case_user_context(case)
        )
        session.chat(case["question"])
        MessageRecorder(recorder_path=run / "session_messages.jsonl").record(
            {"session_id": session.id, "messages": session.messages}
        )
    ...
    while case is not None and current_count <= run_count:
        supervisor_agent = SupervisorAgent(
            ...
            message_recorder=MessageRecorder(
                recorder_path=run / "session_messages.jsonl"
            ),
        )
