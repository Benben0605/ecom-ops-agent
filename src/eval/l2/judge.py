...
- 忠实轴分两道独立工序，别混：
  · 【抽取层】从 answer 挑出要进忠实轴的句子。
    ...
    主观评价、空泛营销赞美（如「超值好物」——注意豁免仅限无真假可言的空话）、定性归纳
    （如「一整套搭配」）、服务承诺/行动引导（如「联系我跟进」）、寒暄粘合剂——都不是事实断言，不抽。
    注意：「这三款都适合敏感肌」这类可证伪的归纳要抽（之后判它有没有据），别因为是归纳就放过。
    【有真假可言的主张不是修辞】涉及销量/热度（「卖得很不错」「热销款」）、用户购买史
    （「你之前买过X」）、店铺活动/优惠、会员身份的具体陈述，都是可证伪断言，必须抽——
    它们不享受营销修辞豁免。
    闸门二=宁多勿漏：把每个可证伪事实点都单独抽出来，哪怕它被包在服务话术/行动引导/寒暄搭话里——
    如「您可以申请退款（未发货可直接取消）」要把『未发货可直接取消』单独抽出来判；
    「对了，看你之前还买过我们家的X，最近Y区也在搞活动」要把『你买过X』『Y区在搞活动』
    两条都抽出来判。别因为外面裹了层口吻就整句放过。抽多无妨（判定层会把有据的判 supported），
    抽漏才致命（真越界会静默逃逸成假阴）。
  · 【判定层】对已抽进来的每条断言，到 tool_outputs 找支撑，判 supported/unsupported。
    ...
    池里特征一步推出的品类/用途/功效，都算支撑（如「防滑」支撑「稳固」、「深层补水」支撑「对付干燥」、
    商品名「助眠香薰」支撑「放松助眠」、「咖啡」支撑「提神」、「坚果」支撑「营养」）。
    【别把用途功效和销量热度连坐】商品的用途/功效外推（该按上条判绿）≠ 销量/热度/购买史类主张
    （池里无据就判红）——同一句话可以同时含两者（如「提神又健康，卖得很不错」），逐条分开判，
    别因为句子里有嫌疑主张就把相邻的功效外推一起判红。
