...
def run():
    ...
    rel_ch, irr_goldq_ch, irr_negq_ch = (
        [],
        [],
        [],
    )  # H3b: 块级分（相关/gold-query非gold/负样本query块）
    ...
    for m in ("vec_top3", "rerank_top3"):
        d = agg[m]
        print(
            f"  {m:14} {avg(d['R@1']):6.2f} {avg(d['R@3']):6.2f} {avg(d['MRR']):6.2f} {avg(d['P@3']):6.2f}"
        )
    ...
    g = sorted(s for _, s in gold_rr)
    n = sorted(s for _, s in neg_rr)
    print(f"  in-scope(n={len(g)}): min={g[0]} ~ max={g[-1]}")
    print(f"  out-scope(n={len(n)}): min={n[0]} ~ max={n[-1]}")
    print(
        f"  → in最低 {g[0]} vs out最高 {n[-1]}：{'分得开 gap=' + str(round(g[0] - n[-1], 4)) if g[0] > n[-1] else '重叠 ' + str(round(n[-1] - g[0], 4))}"
    )

    print("\nH3b grader替代(块级 per-chunk)：相关块 vs 不相关块")
    irr = irr_goldq_ch + irr_negq_ch
    rs = sorted(x[2] for x in rel_ch)
    is_ = sorted(x[2] for x in irr)
    print(
        f"  相关块(n={len(rel_ch)}): min={rs[0]} ~ max={rs[-1]}  中位≈{rs[len(rs) // 2]}"
    )
    print(
        f"  不相关块(n={len(irr)}): min={is_[0]} ~ max={is_[-1]}  中位≈{is_[len(is_) // 2]}"
    )
    leak = sum(1 for s in is_ if s >= rs[0])
    print(
        f"  → 相关最低 {rs[0]} vs 不相关最高 {is_[-1]}：{'分得开' if rs[0] > is_[-1] else '重叠 ' + str(round(is_[-1] - rs[0], 4))}"
        f"；阈值卡相关最低时不相关漏 {leak}/{len(irr)}"
    )
    print("  不相关块 top5 高分(骗过 rerank)：", sorted(irr, key=lambda x: -x[2])[:5])
    print(
        "  相关块 bottom5 低分(rerank 失手)：", sorted(rel_ch, key=lambda x: x[2])[:5]
    )

    (ROOT / "logs" / "rerank_benchmark_result.json").write_text(
        json.dumps(
            {
                "H1": {m: {k: avg(v) for k, v in d.items()} for m, d in agg.items()},
                "H2_cross_family": cross,
                "H3a_query_top1": {"gold": gold_rr, "neg": neg_rr},
                "H3b_per_chunk": {
                    "relevant": rel_ch,
                    "irr_goldq": irr_goldq_ch,
                    "irr_negq": irr_negq_ch,
                },
                "config": {
                    "n_recall": N_RECALL,
                    "top_k": TOP_K,
