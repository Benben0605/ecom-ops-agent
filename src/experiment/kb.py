...
def _run_cases_once() -> dict:
    ...
    cases = {
        c["id"]: c
        for c in json.loads(
            (ROOT / "data" / "eval_cases.json").read_text(encoding="utf-8")
        )
    }
    run_map = []
    for cid in KB_CASES:
        s = ChatSession(
            audit_recorder=AuditRecorder(recorder_path=LOGS / "audit.jsonl")
        )
...
def run_experiment(n: int = 2):
    ...
    # 备份用户 logs
    bak = {
        p: (LOGS / p).read_text(encoding="utf-8") if (LOGS / p).exists() else None
    ...
    try:
        for cfg in CONFIGS:
            ...
            acc = {
                cid: {"hit": 0, "hit_tot": 0, "faith": 0, "faith_tot": 0}
                for cid in KB_CASES
            }
            for _ in range(n):
                ...
                for cid, s in scores.items():
                    if not s:
                        continue
                    acc[cid]["hit"] += s["hit_ok"]
                    acc[cid]["hit_tot"] += s["hit_total"]
                    acc[cid]["faith"] += s["faith_ok"]
                    acc[cid]["faith_tot"] += s["faith_total"]
...
def _agg(per_case, key, tot):
    s = sum(per_case[c][key] for c in per_case)
    t = sum(per_case[c][tot] for c in per_case)
