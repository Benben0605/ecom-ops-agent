...
def write_reports(rows: list[dict]):
    ...
    for r in rows:
        ...
        for t in r["kb_calls"]:
            ...
            lines.append(
                f"- kb param「{t['query']}」→ 留 {t['kept']}/{len(t['chunks'])}　{mark}"
            )
            for c in t["chunks"]:
...
if __name__ == "__main__":
    ...
    ab = [
        (
            r["case_id"],
            t["query"],
            "全部弃: " + "/".join(c["item"] for c in t["chunks"]),
        )
        for r in rows
        for t in r["kb_calls"]
        if t["outcome"] == "abstain"
    ]
    print(
        f"观测 {len(rows)} 个 kb case，报告 → logs/kb_observe.md（人读）/ .json（程序读）"
    )
