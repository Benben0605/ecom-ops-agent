...
def run(
    category: str | None = None, metric: str | None = None, role: str | None = None
) -> str:
    ...
    if category:
        orders = [
            o for o in orders if category in o["category"] or o["category"] in category
        ]
    if not orders:
    ...
    hot_count_lines = "按订单数口径：\n" + "\n".join(
        f"{i}. {item}（{n} 单）" for i, (item, n) in enumerate(hot_count, 1)
    )
