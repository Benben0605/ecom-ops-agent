...
_PRODUCTS = json.loads(
    (Path(__file__).parents[2] / "data" / "products.json").read_text()
)


def run(
    category: str | None = None, budget: float | None = None, user_id: str | None = None
) -> str:
    ...
    hits = [
        p for p in _PRODUCTS if category in p["category"] or p["category"] in category
    ]
